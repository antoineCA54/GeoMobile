...
st_intersects( 
		st_buffer( st_transform 
		(st_setSRID
			(st_makepoint(".$_POST['lat'].",".$_POST['lng']."),4326),310024140),2000)
...
st_x(st_centroid(st_transform((p.g),4326))) as x
FROM 
bdaplat_nt.sb_geom_84 p 
JOIN bdaplat_nt.sb_data_84 d USING (rowid) 
WHERE 
st_isvalid(p.g) and
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=="afficher_parcelle")
	{
		$idcom=connex2("bdaplat_nt","myparam");
		// Requete version V9 Dynmap
		 
		$requete="SELECT ST_AsGeoJSON(st_transform(iep_par_geom,4326)),
st_y(st_centroid(st_transform((iep_par_geom),4326))) as y,
st_x(st_centroid(st_transform((iep_par_geom),4326))) as x,
d.exp_pacage || ', ' || d.exp_raisonsociale as d FROM
public.parcelles_2015 d WHERE iep_ilo_comlib='".$_POST['rowid']."'";
echo $requete;
		$result=pg_query($idcom,$requete);
